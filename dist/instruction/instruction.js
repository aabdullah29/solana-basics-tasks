...
const callCounter = (parentAccount) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const dataAccount = new web3_js_1.PublicKey("GTWB5LaF7jBEhan4FYpoRR5gRMKbGs379WcATADLtKMJ");
    const buffers = [Buffer.from(Int8Array.from([2])), numberToBuffer(2800)];
    const data = Buffer.concat(buffers);
    const instruction_1 = new web3_js_1.TransactionInstruction({
        // keys: [{pubkey: dataAccount.publicKey, isSigner: false, isWritable: true}],
        keys: [{ pubkey: dataAccount, isSigner: false, isWritable: true }],
        programId: new web3_js_1.PublicKey(CONTRACT_PROGRAM_ID),
        data: data
    });
    // const instruction_2 = new TransactionInstruction({
    //     keys: [{pubkey: dataAccount, isSigner: false, isWritable: true}],
    //     programId: new PublicKey(CONTRACT_PROGRAM_ID),
    //     data: Buffer.concat([Buffer.from(Int8Array.from([1]))])
    // });
    // const instruction_3 = new TransactionInstruction({
    //     keys: [{pubkey: dataAccount, isSigner: false, isWritable: true}],
    //     programId: new PublicKey(CONTRACT_PROGRAM_ID),
    //     data: Buffer.concat([Buffer.from(Int8Array.from([0]))])
    // });
    // const instruction_4 = new TransactionInstruction({
    //     keys: [{pubkey: dataAccount, isSigner: false, isWritable: true}],
    //     programId: new PublicKey(CONTRACT_PROGRAM_ID),
    //     data: Buffer.concat([Buffer.from(Int8Array.from([0]))])
    // });
    ...
    yield (0, web3_js_1.sendAndConfirmTransaction)(connection, transaction, [parentAccount]);
    // Read data
